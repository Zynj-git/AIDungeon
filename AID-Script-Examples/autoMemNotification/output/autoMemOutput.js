...
const modifier = (text) =>
{
    if (worldEntries.length != state.copyWorldEntriesLength && !state.blockOutput) // Detect and assemble the new auto-generated entries into an array that is present on consecutive, invalidated turns.
    {
        state.manageEntries = [] // Create a clean slate for when rebuilding the list.
        const newEntriesCount = (worldEntries.length - state.copyWorldEntriesLength) // For indexing, find out by how many entries they deviate from each other.
    ...
    if (getHistoryType(-1) == 'continue' && state.blockOutput) {state.blockOutput = false; delete state.message; state.manageEntries = []; state.copyWorldEntriesLength = worldEntries.length;} // Permit a continue action to unblock.
